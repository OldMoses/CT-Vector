...
'² º º                                                                            º º ²
'² º º Development and beta test version 0.50  uploaded __-__-2021                º º ²
'² º º                                                                            º º ²
...
'                                                               INITIAL PARAMETERS
VerA = 0: VerB = 5: VerC = 0 '                                  version tags
ttl = "CT Vector 0.50" '                                        Title bar string
...
SUB Build_Str (main AS STRING, allow AS STRING, kp AS STRING)
    ...
    SELECT CASE kp '                                            keypress as case selection
        CASE IS = CHR$(8) '                                     if backspace pressed
            IF LEN(main) > 0 THEN '                             and main string exists
                main = LEFT$(main, LEN(main) - 1) '             remove rightmost character
            END IF '
        CASE ELSE '                                             if anything other than backspace
            IF INSTR(allow, kp) <> 0 THEN '                     if an allowable character has been pressed
                main = main + _TRIM$(kp) '                      add it to main string
            END IF '
    END SELECT '
    kp = "" '                                                   clear the input
...
FUNCTION Choose_World% (header AS STRING, mode AS _BYTE)
    ...
    'called from: Flight_Plan, Mouse_Button_Left
...
SUB Comments
    ...
    'transformed by any applied rotations around the X axis.
...
SUB Dialog_Box (heading AS STRING, xsiz AS INTEGER, ysiz AS INTEGER, ypos AS INTEGER, bcol AS _UNSIGNED LONG, tcol AS _UNSIGNED LONG)
    ...
    _PUTIMAGE (_SHR(_WIDTH, 1) - _SHR(_WIDTH(dbox&), 1), ypos), dbox& 'display box over calling destination image
...
SUB Draw_Ring_Belt (cntr AS V3, rng AS _INTEGER64, in AS _INTEGER64, rat AS SINGLE)
    ...
    ELSE
        cen.pY = cntr.pY * rat
        d2a## = PyT(2, cntr, origin) '                          distance from orbit track center to active unit MOVED OUT LOOP
        Vec_Cross drct, cntr, khat '                            get orthogonal of vertical and orbit track radius MOVED OUT LOOP
        Vec_Mult drct, rat '                                                                           MOVED OUT LOOP
...
SUB Flight_Plan
    ...
    IF NOT flt% THEN
        Con_Blok 545, 100, 160, 48, "Launch", 1, _RGBA32(22, 166, 211, 255)
        Con_Blok 545, 164, 160, 48, "Goto & Land", 1, _RGBA32(22, 166, 211, 255)
        Con_Blok 545, 420, 160, 48, "Abort [Esc]", 0, _RGBA32(22, 166, 211, 255)
    ELSE
        Con_Blok 545, 100, 160, 48, "Jump in Matching", 1, _RGBA32(22, 166, 211, 255)
        Con_Blok 545, 164, 160, 48, "Goto & Land", 1, _RGBA32(22, 166, 211, 255)
        Con_Blok 545, 228, 160, 48, "Orbit", 1, _RGBA32(22, 166, 211, 255)
        Con_Blok 545, 292, 160, 48, "Hold station", 1, _RGBA32(22, 166, 211, 255)
        Con_Blok 545, 356, 160, 48, "Safe Jump", 1, _RGBA32(22, 166, 211, 255)
        Con_Blok 545, 420, 160, 48, "Abort [Esc]", 0, _RGBA32(22, 166, 211, 255)
    END IF
    _DISPLAY
    ...
    DO
        ...
        IF ms AND 1 THEN
            SELECT CASE _MOUSEX
                CASE 545 TO 705
                    SELECT CASE _MOUSEY
                        CASE 100 TO 148 '                       planet matching / launch
                            choice% = 1: dl% = -1
...
SUB Get_Body_Vec (var AS INTEGER, var2 AS V3)
    ...
    IF r% > 1 THEN
        DO
            ...
            u% = Find_Parent(u%) '                              increment to parent
    ...
    ELSE
        ...
        DO '                                                    Azimuth display loop
            ...
            IF ms AND 1 THEN
                ...
                Clear_MB 1: in = -1
            END IF
            IF ms AND 512 THEN ws! = ws! - .5
            IF ms AND 1024 THEN ws! = ws! + .5
...
SUB Re_Calc
    ...
    IF units = 0 THEN vpoint = 0: d = 0 ELSE d = 1 '            if no ships then set origin of VCS at primary
    c = d: exists = 0
...
SUB Remove_Units (mode AS _BYTE)
    ...
    IF units > 0 THEN
        ...
        Con_Blok 480, 225, 120, 32, "Yes [enter]", 0, clr&(4)
        Con_Blok 660, 225, 120, 32, "No [Esc]", 0, clr&(4)
        _DISPLAY
...
SUB System_Map (g AS SINGLE)
    ...
    FOR p% = 1 TO orbs '                                        Iterate through all system bodies
        ...
        END IF
        '                                                       If we've made it to here we're going to draw something of p%
...
SUB Text_Bubble (xpos AS INTEGER, ypos AS INTEGER, remark AS STRING)
    ...
    FOR y% = 0 TO 1 '                                            draw bounding box
        IF y% = 0 THEN c& = Black ELSE c& = White
